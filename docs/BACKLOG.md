...
## TASK-006 · 情报未闭环消费（缺"组合-推导-验证追踪"机制）🟡进行中

**优先级**:高(项目初心"把漏洞穿成链"的总病根)

**发现时间**:2026-07-21(实战记录:收集了 73 路径/EagleEye 头/token/UUID,但各自停在"发现"层,没交叉组合成一次真实请求)

**背景**:
engine 到目前为止做的都是"优化输入"(JS 挖全、base 拼对、摆事实提问),但没有一个东西
负责"强制消费"。intel 各字段(backends/endpoints/secrets/hosts)平行躺着,互不组合。
AI 于是退化为"顺序执行":挖完 JS→发个包→429→记 finding→停,而非"组合-推导-反馈循环"。

实战三大症状(AI 自述):
- EagleEye 头信息在 js_assets.json 里看到了,但没形成"这是请求必要组成部分"的推导,第 N 次才加。
- 429 触发后只注册 finding 就停,没降频重试/换组合/跟限流窗口。
- 没回 JS 搜 login 源码看 axios 拦截器完整配置,只停在 URL 路径层。

**根因**:缺"情报组合-消费-反馈"闭环。AI 自己开的药方即"情报 × 攻击向量 = 动作矩阵"+
强制三问(还能和什么组合?组合成什么请求?发过没有?)。证明光靠提问(层次A)不够,需装配台(层次B)。

**方案(层次 B,已定)**:
- **组合装配(engine 机械活)**:读 intel 的 backends×endpoints×headers×secrets,按有意义的规则
  撮合成"完整请求候选"(base+prefix+path+建议头/token),不全排列爆炸、不自动发包。
- **消费追踪**:记录每个候选"发过没有"(consumed 思想从单条情报升级到情报组合)。
- **反馈触发**:新情报/新响应入库时,主动提示"它能和哪些旧情报组合成新链"。
- **边界**:engine 只摆"装配好的候选链 + 为什么这么配 + 发没发过",打不打由 AI 判断(守骨架宪法)。

**验证数据**:本次实战正是依据;实现后回测"是否不再遗漏组合"。
